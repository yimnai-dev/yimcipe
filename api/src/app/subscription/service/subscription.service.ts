...
export class SubscriptionService {
  ...
  subscribeOrUnsubscribe = async (userId: string, subId: string) => {
    ...
    const subscriberExists = await this.userModel.findOne({
      where: { userId: subId },
    });
    if (!subscriberExists) {
      return {
    ...
    const unsubIfSubbed = await this.subscriberModel.findOne({
      where: { userId: userId, subscriberId: subId },
    });
    if (unsubIfSubbed) {
      const unsubbed = await this.subscriberModel.destroy({
        where: { userId: userId, subscriberId: subId },
      });
      if (!unsubbed) {
        return {
    ...
    const subIfNoSub = await this.subscriberModel.create({
      userId: userId,
      subscriberId: subId,
    });
    if (!subIfNoSub) {
